...
import type { LinkType, LinkVisibility } from '@/app/lib/types'
import { linksReducer, type LinksAction } from '@/app/reducers/linksReducer'
...
export const LinksProvider = ({ children, targetUsername, isOwnProfile }: { children: ReactNode; targetUsername?: string; isOwnProfile?: boolean }) => {
	...
	}, [targetUsername, isSignedIn, isOwnProfile, getToken])
	...
	const toggleLinkVisibility = async (id: string) => {
		try {
			...
			const newVisibility: LinkVisibility = link.visibility === 'public' ? 'private' : 'public'
	...
	return (
		<LinksContext.Provider value={{ links, loading, error, isOwnProfile: isOwnProfile || false, dispatch, addLink, editLink, deleteLink, moveLink, toggleLinkVisibility, refetch }}>
			{children}
